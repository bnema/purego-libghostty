...
func TestOpenRejectsZeroHandleWithoutError(t *testing.T) {
	var paths []string
	ops := Ops{
	...
	if handle, err := Open(ops, "", "zero.so", "valid.so"); err != nil || handle != 42 {
		t.Fatalf("Open() = (%d, %v), want (42, nil)", handle, err)
	}
	if !reflect.DeepEqual(paths, []string{"zero.so", "valid.so"}) {
		t.Fatalf("paths = %#v", paths)
	}
}
